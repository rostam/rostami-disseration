...
\begin{itemize}
  \item Apply automaic differentation to compute Jacobian-vector product $J \vek{z}$
  \item Choose a block size $k$ and apply the sparsification to get \sparsify{J}. Assemble \sparsify{J} via automatic differentiation and store it explicitly.
  \item Construct a preconditioner $M$ from \sparsify{J} by performing an SILU on each block of~\sparsify{J}. 
\end{itemize}
\end{frame}
...
A minimal $k$ would decreae the computational overhead.
...
$R_init$, the distance-$2$ coloring restricted to $R_init$ is the mapping 
...
(L{\"u}lfesmann: a doctorate of Prof. B{\"u}cker)
\begin{itemize}
\item Compute $R_{init} = \rho(J)$
\item Compute $R_{init} \cup Fill-in = SILU(R_{init})$
...
In the right matrix, one nonrequired element survives.
In the left matrix, three nonrequired elements survive.
